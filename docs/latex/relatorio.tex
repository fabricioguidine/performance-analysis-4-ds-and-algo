...
To ensure the correctness and reliability of all implemented algorithms and data structures, a comprehensive test suite was developed using JUnit 4. The testing framework validates all components of the project.
...
\begin{itemize}
    \item \textbf{Model Classes}: Tests for \texttt{Record} and \texttt{Author} classes, validating constructors, getters, setters, and comparison methods
    \item \textbf{Sorting Algorithms}: Unit tests for QuickSort and HeapSort, verifying correctness for various input sizes and edge cases (empty arrays, single elements, already sorted, reverse sorted, random order)
    \item \textbf{Hash Tables}: Tests for \texttt{RecordHashTable} and \texttt{AuthorHashTable}, validating insertion, search, resize operations, and collision handling
    \item \textbf{Tree Structures}: Tests for Red-Black Tree and B+ Tree implementations, validating insertion, search, and balance properties
    \item \textbf{I/O Utilities}: Tests for file reading and writing operations, ensuring correct data parsing and output generation
\end{itemize}
...
\begin{itemize}
    \item \texttt{make test} (Linux/Mac)
    \item \texttt{./test.sh} (Linux/Mac)
    \item \texttt{test.bat} (Windows)
\end{itemize}

The test suite ensures that all implementations maintain correctness across different scenarios and edge cases, providing confidence in the experimental results presented in this report.
